...
def read_datasets():
    ...
    genuine_users = pd.read_csv(r"C:\Users\91781\Desktop\Genuine-users.csv")
    fake_users = pd.read_csv(r"C:\Users\91781\Desktop\fake-users.csv")
...
data['statuses_count'].fillna('', inplace=True)
data['followers_count'].fillna('', inplace=True)
data['friends_count'].fillna('', inplace=True)
data['time_zone'].fillna('', inplace=True)
data['location'].fillna('', inplace=True)
data['no_of_times_logins'].fillna('', inplace=True)
data['no_times_pass_change'].fillna('', inplace=True)
data['description'].fillna('', inplace=True)


# Combine features
data['combined_features'] = data['name'] +' ' +data['statuses_count'].astype(str) + ' ' + data['followers_count'].astype(str) + ' ' + data['friends_count'].astype(str) + ' ' + data['time_zone'] + ' ' + data['location'] + ' ' + data['no_of_times_logins'].astype(str) + ' ' + data['no_times_pass_change'].astype(str) + ' ' + data['description'] 
...
def main():
    ...
    statuses_count=st.text_area('Enter statuses count:')
    followers_count = st.text_input('Enter followers count:')
    friends_count = st.text_input('Enter friends count:')
    time_zone=st.text_area('Enter Time_zone:')
    location = st.text_input('Enter location:')
    no_of_times_logins=st.text_area('Enter number of logins:')
    no_times_pass_change=st.text_area('Enter number of times password changed:')
    ...
    if st.button('Predict'):
        ...
        combined_features = name + ' ' + statuses_count + ' ' + followers_count + ' ' + friends_count + ' ' + time_zone + ' ' + location + ' ' + no_of_times_logins + ' ' + no_times_pass_change + ' ' + description
